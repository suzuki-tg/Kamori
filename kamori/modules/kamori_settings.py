...
@loader.tds
class KamoriSettingsMod(loader.Module):
    """Advanced settings for Kamori Userbot"""

    strings = {"name": "KamoriSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"kamori-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "kamori" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_kamori(self, message: Message):
    ...
    async def weburl(self, message: Message, force: bool = False):
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.kamori.web.get_url(proxy_pass=False),
                },
                gif="https://t.me/kamori_assets/28",
            )
            return
        ...
        if not main.kamori.web:
            main.kamori.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.kamori.api_token,
                proxy=main.kamori.proxy,
                connection=main.kamori.conn,
            )
            await main.kamori.web.add_loader(self._client, self.allmodules, self._db)
            await main.kamori.web.start_if_ready(
                len(self.allclients),
                main.kamori.arguments.port,
                proxy_pass=main.kamori.arguments.proxy_pass,
            )
        ...
        url = await main.kamori.web.get_url(proxy_pass=True)

        await form.edit(
            self.strings("tunnel_opened"),
            reply_markup={"text": self.strings("web_btn"), "url": url},
            gif="https://t.me/kamori_assets/48",
        )
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._kamori_entity_cache)} cache records"
                )
                self._client._kamori_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._kamori_fulluser_cache)} cache records"
                )
                self._client._kamori_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._kamori_fullchannel_cache)} cache"
                    " records"
                )
                self._client._kamori_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._kamori_perms_cache)} cache records"
                self._client._kamori_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._kamori_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._kamori_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._kamori_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._kamori_fulluser_cache = {}
                self._client._kamori_fullchannel_cache = {}
                self._client.kamori_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._kamori_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._kamori_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._kamori_fullchannel_cache)} records\nLoader"
