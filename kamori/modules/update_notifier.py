...
class UpdateNotifier(loader.Module):
    """Tracks latest Kamori releases, and notifies you, if update is required"""
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "kamori/update"},
                {"text": self.strings("ignore"), "data": "kamori/ignore_upd"},
            ]
        )
    ...
    async def poller(self):
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            m = await self.inline.bot.send_animation(
                self.tg_id,
                "https://github.com/suzuki-tl/assets/raw/master/kamori_update.mp4",
                caption=self.strings("update_required").format(
                    utils.get_git_hash()[:6],
                    '<a href="https://github.com/suzuki-tl/Kamori/compare/{}...{}">{}</a>'
                    .format(
    ...
    async def update(self, call: InlineCall):
        ...
        if call.data not in {"kamori/update", "kamori/ignore_upd"}:
            return

        if call.data == "kamori/ignore_upd":
