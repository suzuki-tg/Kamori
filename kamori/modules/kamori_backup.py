...
@loader.tds
class KamoriBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "KamoriBackup"}

    async def client_ready(self):
        if not self.get("period"):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="https://github.com/suzuki-tl/assets/raw/master/unit_alpha.png",
        ...
        self._backup_channel, _ = await utils.asset_channel(
            ...
            silent=True,
            archive=True,
            avatar="https://github.com/suzuki-tl/assets/raw/master/kamori-backups.png",
            _folder="kamori",
        )
    ...
    async def handler(self):
        try:
            ...
            backup.name = (
                f"kamori-db-backup-{datetime.datetime.now():%d-%m-%Y-%H-%M}.json"
            )
            ...
            logger.exception("KamoriBackup failed")
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["kamori.inline"].pop("bot_token")
