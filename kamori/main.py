...
class Kamori:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("kamori-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def save_client_session(self, client: CustomTelegramClient):
        ...
        client.kamori_db = database.Database(client)
        await client.kamori_db.init()
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            await client.kamori_inline.bot.send_animation(
                logging.getLogger().handlers[0].get_logid_by_client(client.tg_id),
                "https://github.com/suzuki-tl/assets/raw/master/kamori_banner.mp4",
                caption=(
                    "🌲 <b>Kamori {} started!</b>\n\n🌳 <b>GitHub commit SHA: <a"
                    ' href="https://github.com/suzuki-tl/Kamori/commit/{}">{}</a></b>\n✊'
                    " <b>Update status: {}</b>\n<b>{}</b>".format(
                        ".".join(list(map(str, list(__version__)))),
                        build,
                        build[:7],
            ...
            logging.debug(
                ...
                client.tg_id,
                client.kamori_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
