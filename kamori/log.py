...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Kamori and Dragon modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (
                module.startswith("kamori.modules")
...
class TelegramLogsHandler(logging.Handler):
    ...
    def _gen_web_debug_button(self, item: KamoriException) -> list:
        if not item.sysinfo:
            return []
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], KamoriException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_kamori_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_kamori_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        record.kamori_caller = caller

        if record.levelno >= self.tg_level:
            if record.exc_info:
                exc = KamoriException.from_exc_info(
                    *record.exc_info,
