...
async def answer(
    ...
    **kwargs,
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "kamori_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "kamori")
        except Exception:
            folder = None
...
def get_commit_url() -> str:
    """
    Get current Kamori git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return (
            f'<a href="https://github.com/suzuki-tl/Kamori/commit/{hash_}">#{hash_[:7]}</a>'
        )
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/suzuki-tl/Kamori/commit/{hash_}" if hash_ else "",
    )
