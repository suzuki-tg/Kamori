...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.kamori_me.id
        self._cached_usernames = [
            (
                self._client.kamori_me.username.lower()
                if self._client.kamori_me.username
                else str(self._client.kamori_me.id)
            )
        ]

        self._cached_usernames.extend(
            getattr(self._client.kamori_me, "usernames", None) or []
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("kamori-")
            and message.chat.title != "kamori-logs"
        ):
            if not watcher:
