...
class InlineManager(
    ...
    :type client: kamori.tl_cache.CustomTelegramClient
    :type db: kamori.database.Database
    :type allmodules: kamori.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("kamori.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.kamori_me)
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start kamori init")
        except (InputUserDeactivatedError, ValueError):
            self._db.set("kamori.inline", "bot_token", None)
            self._token = False
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start kamori init"
                )
            except Exception:
