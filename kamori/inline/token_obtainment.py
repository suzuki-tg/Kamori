...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("kamori.inline", "custom_bot", False):
                username = self._db.get("kamori.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
                    ...
                    uid = utils.rand(6)
                    username = f"@kamori_{uid}_bot"
            else:
                uid = utils.rand(6)
                username = f"@kamori_{uid}_bot"

            for msg in [
                f"🌲 Kamori Userbot of {self._name}"[:64],
                username,
                "/setuserpic",
                username,
            ]:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if self._db.get(
                        "kamori.inline", "custom_bot", False
                    ) and self._db.get(
                        "kamori.inline", "custom_bot", False
                    ) != button.text.strip(
                        "@"
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@kamori_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("kamori.inline", "bot_token", token)
                    self._token = token
