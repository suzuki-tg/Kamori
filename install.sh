...
runin() {
	...
	{ "$@" 2>>../kamori-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../kamori-install.log
	done
}
...
printf "\n\e[1;35;47m                         \e[0m"
printf "\n\e[1;35;47m 88  dP    db    8b    d8 \e[0m"
printf "\n\e[1;35;47m 88odP    dPYb   88b  d88 \e[0m"
printf "\n\e[1;35;47m 88\"Yb   dP__Yb  88YbdP88 \e[0m"
printf "\n\e[1;35;47m 88  Yb dP\"\"\"\"Yb 88 YY 88 \e[0m"
printf "\n\e[1;35;47m                         \e[0m"
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" kamori-install.log
fi

if [ -d "Kamori/kamori" ]; then
	cd Kamori || {
...
if [ -f ".setup_complete" ]; then
	# If kamori is already installed by this script
	PYVER=""
...
echo "Installing..." >kamori-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/suzuki-tl/Kamori/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Kamori
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/suzuki-tl/Kamori/ || {
